...
public class JsCallJava {
    ...
    public String call(ExtendWebView webView, String jsonStr) {
        if (!TextUtils.isEmpty(jsonStr)) {
            try {
                JSONObject callJson = weiuiJson.parseObject(jsonStr);
                ...
                for (String key : mMethodsMap.keySet()) {
                    if (key.startsWith(methodName + "_")) {
                        ...
                        while (m.find()) {
                            ...
                            switch (m.group()) {
                                case "_S":
                                    values[k + 1] = weiuiParse.parseStr(temp);
                                    break;
                                ...
                                case "_N":
                                    ...
                                    if (ccls == int.class) {
                                        values[k + 1] = weiuiParse.parseInt(temp);
                                    } else if (ccls == long.class) {
                                        values[k + 1] = weiuiParse.parseLong(temp);
                                    } else {
                                        values[k + 1] = weiuiParse.parseDouble(temp);
                                    }
                                    break;

                                case "_B":
                                    values[k + 1] = weiuiParse.parseBool(temp);
                                    break;

                                case "_O":
                                    values[k + 1] = weiuiJson.parseObject(temp);
                                    break;

                                case "_F":
                                    values[k + 1] = new JsCallback(webView, mInjectedName, weiuiParse.parseInt(temp));
                                    break;
